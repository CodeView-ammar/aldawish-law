    ...
    public static function table(Table $table): Table
    {
        return $table
            ...
            ->actions([
                ...
                Tables\Actions\DeleteAction::make()->disabled(fn ($record) => in_array($record->name, ['manager', 'operator']))->before(function (Tables\Actions\DeleteAction  $action, Role $role) {
                    if ($role->users()->count() > 0) {
                        Notification::make()
                            ->warning()
                            ->title(__('panel.messages.warning'))
                            ->body(__('panel.messages.role_has_many_users'))
                            ->persistent()
                            ->send();
                        $action->cancel();
                    }
